...
(function(Hooks) {
  ...
  setCategoryPrivileges = function(category, groupName, callback) {
    return async.waterfall([
      function(next) {
        ...
        removePrivileges = ['read', 'topics:read', 'topics:create', 'topics:reply', 'topics:tag', 'posts:edit', 'posts:delete', 'topics:delete', 'upload:post:image'];
  ...
  return Hooks.action.groupUpdate = function(obj) {
    var name, oldName, payload, updateCategoryInfo, values;
    oldName = obj.oldName, values = obj.values;
    ...
    updateCategoryInfo = function(cb) {
      return async.waterfall([
        function(next) {
          return db.getObjectFields('group:' + name, ['cid', 'slug'], next);
        }, function(data, next) {
          if (data.cid) {
            return db.setObject("category:" + data.cid, {
              groupName: name,
              groupSlug: data.slug
            }, next);
          } else {
